...
def inspect(proc,case,directory):
            ...
            for page in pages:
                ...
                try:
                    with Image.open(page['image']) as im:
                        ...
                        page['precheck']={'status':'passed' if (not is_blank and aspect_valid and res_valid) else 'review','isUniform':is_blank,'aspectRatio':aspect,'size':list(im.size),'suggestedRubric':{k:'符合' for k in RUBRIC} if (not is_blank and aspect_valid and res_valid) else {k:'待审' for k in RUBRIC}}
                ...
                source_page=None if 'timestampSeconds' in page['mapping'] else page['page']
    ...
    atomic(root/'visual-pages.json',{'caseId':case['id'],'sourceSha256':case['source']['sha256'],**case['options'],'pages':rows,'errors':errors,'status':'review' if rows else 'blocked'})
    return {'metrics':rows,'errors':errors,'options':case['options'],'sourceFormat':case.get('format'),'actualEngine':(proc.get('payload') or {}).get('engine'),'actualRoute':(proc.get('payload') or {}).get('route'),
            ...
            'rubric':RUBRIC,'reviewTemplate':str(root/'visual-review-template.json'),'status':'review' if rows else 'blocked',
            'note':'All PDF pages; five temporal video samples, not all frames; no pixel release threshold; optional independent GT remains subject to human review'}
